...
# v12: compute Hamming distances one index UMI at a time instead of building
# the full N x N UMI distance matrix. This keeps the output format unchanged
# while reducing memory usage for high-UMI targets.
umi_df_mod <- umi_df
umi_corre <- data.frame(original_umi = character(), index_umi = character(), hamming_dist = numeric(), stringsAsFactors = FALSE)
...
if (nrow(umi_df_mod) == 1) {
  ...
  while (nrow(umi_df_mod) > 0) {
    ...
    index_vec <- umi_matrix[, index_umi]

    diff_with_index <- umi_length - as.numeric(t(umi_matrix[, umi_df_mod$umi, drop = FALSE]) %*% index_vec)
    names(diff_with_index) <- umi_df_mod$umi
    ...
    umi_corre <- rbind(umi_corre, data.frame(
      ...
      hamming_dist = as.numeric(diff_with_index[index_umi_cluster]),
      stringsAsFactors = FALSE
    ))
  }
}
